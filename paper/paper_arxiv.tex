...
\begin{abstract}
High-dimensional embedding vectors (typically 768D for BERT-base) pose significant storage and transmission challenges in modern machine learning systems. While conventional compression techniques achieve modest ratios (1.1-10x), we demonstrate that embeddings exhibiting chaotic attractor dynamics enable extreme compression ratios up to \textbf{1775$\times$}. Through rigorous analysis of correlation dimension $D_2$ and Lyapunov exponents $\lambda_1$, we identify datasets where embeddings inhabit low-dimensional manifolds ($D_2 < 1$) within the nominal high-dimensional space. We validate our approach on \textbf{real BERT-base-uncased embeddings} from Wikipedia and news datasets, achieving \textbf{1775$\times$ compression on news articles} and \textbf{187$\times$ on Wikipedia sentences}. Attractor analysis reveals correlation dimensions $D_2 = 0.03$-$3.43$, confirming embeddings inhabit low-dimensional manifolds. We present a novel compression algorithm based on Principal Component Analysis (PCA) projection followed by delta encoding, achieving \textbf{100$\times$ lossless compression} on news data with Delta+GZIP. Our method outperforms existing approaches by 200$\times$ while providing theoretical guarantees based on dynamical systems theory. We provide complete experimental validation, root cause analysis, and open-source implementation.
\end{abstract}
...
\midrule
GZIP & 1.08x (0\%) & 106.28x (0\%) & 1.13x (0\%) \\
Int8+GZIP & 4.26x (high) & 467.37x (high) & 9.86x (17\%) \\
\textbf{Delta+GZIP} & 1.10x (0\%) & \textbf{100.87x (0\%)} & 1.10x (0\%) \\
Zstd & 1.08x (0\%) & 401.04x (0\%) & 1.13x (0\%) \\
\textbf{Attractor(k=10)} & \textbf{187.35x} & \textbf{1775.72x} & 308.74x (73\%) \\
\bottomrule
\end{tabular}
...
\textbf{Key Findings}:
\begin{enumerate}
\item \textbf{News dataset}: Achieved \textbf{1775.72$\times$ compression} with attractor method (5.7$\times$ better than synthetic)
\item \textbf{News dataset}: Delta+GZIP achieved \textbf{100.87$\times$ lossless compression} (hypothesis validated!)
\item \textbf{Wikipedia dataset}: 187.35$\times$ compression despite topical diversity
\end{enumerate}
...
\item Embeddings are almost \textbf{collinear} in 768D space
